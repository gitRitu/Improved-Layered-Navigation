...
namespace Dotsquares\Megamenu\Block\Html;
...
use Dotsquares\Megamenu\Helper\Data as WpHelper;
...
class Topmenu extends \Magento\Theme\Block\Html\Topmenu
{
    ...
    protected function _getHtml(
        ...
        if (!$this->_scopeConfig->getValue(
            'dotsquares_megamenu/megamenu/enable',
        ...
        foreach ($children as $child) {
            ...
            if ($childLevel == 1) {
                ...
                $dynamicSubcategories = (boolean)$parent->getDotsquaresMmDynamicScFlag() && (in_array($parent->getDotsquaresMmDisplayMode(), ['sectioned', 'fullwidth']));
                $columnGroups = explode(",", $parent->getDotsquaresMmDynamicScOpts());
                 ...
                 */
                if ($parent->getDotsquaresMmDisplayMode() != 'default' && $counter == 1) {
            ...
            $categLabelText = $child->getData('dotsquares_mm_label_text');
            $categLabelPostion = $child->getData('dotsquares_mm_label_position');
            ...
            $forceWidth = $childLevel == 1 && $parent->getDotsquaresMmDisplayMode() == 'sectioned' && $width != '' ? $width : '';

            $html .= '<li ' . $this->_getRenderedMenuItemAttributes($child) . ' ' . $hasChildren . ' ' . $forceWidth . ' >';
            if (($childLevel >= 1) && $parent->getData('dotsquares_mm_image_enable')) {
                $categImage = $child->getData('dotsquares_mm_image');
                $categImageAlign = ($parent->getData('dotsquares_mm_image_name_align')) ? $parent->getData('dotsquares_mm_image_name_align') : 'center';
                $categImagePosition = $parent->getData('dotsquares_mm_image_position');
                $categImageAlt = $child->getData('dotsquares_mm_image_alt');
                $categImageContent = '';
                if ($categImage) {
                    $mediaUrl = $this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA);
                    $categImageUrl = str_replace(['media//media', 'media/media'], ['media'], str_replace(['/pub/media'], [''], $mediaUrl) . $categImage);
                    $categImageHeight = $parent->getData('dotsquares_mm_image_height');
                    $categImageWidth = $parent->getData('dotsquares_mm_image_width');
                ...
                $html .= '<a' . $openInNewTab . ' href="' . $child->getUrl() . '" ' . $outermostClassCode . '>'
                    ...
                    . '</span>' . ($categLabelText ? '<span class="ui-menu-icon megamenu-promo-text uppertag megamenu-promo-level'
                ...
                $html .= '<a' . $openInNewTab . ' href="' . $child->getUrl() . '" '
                    ...
                    . '</span>' . ($categLabelText ? '<span class="ui-menu-icon megamenu-promo-text _testing megamenu-promo-level'
                    . $childLevel . '-' . $itemPositionClassPrefix . $counter . '">' : '')
                    . ($categLabelText ? $categLabelText . '</span> ' : '')
                    . '</a>' . $this->_addSubMenu($child, $childLevel, $childrenWrapClass, $limit)
                    . '</li>';
             ...
             */
            if ($parent->getDotsquaresMmDisplayMode() != 'default' && $liGroup && $childLevel == 1) {
             ...
             */
            if ($childLevel == 1 && $parent->getDotsquaresMmDisplayMode() != 'default' && $counter == $childrenCount) {
                $html .= '<span class="close columns-group last"></span>';
                $html .= '</ul>';
            }
            $categImageRadius = $parent->getData('dotsquares_mm_image_radius');
            ...
            $categoryFontColor = $child->getDotsquaresMmFontColor();
            $categoryFontHoverColor = $child->getDotsquaresMmFontHoverColor();
            $categoryFontTextShadow = false;
            $categLabelTextColor = $child->getData('dotsquares_mm_label_font_color');
            $categLabelBackgorundColor = $child->getData('dotsquares_mm_label_background_color');
    ...
    protected function _addSubMenu($child, $childLevel, $childrenWrapClass, $limit)
    {
        ...
        if ($childLevel == 0) {
            $megaMenuClass = $child->getDotsquaresMmDisplayMode() ? $child->getDotsquaresMmDisplayMode() : 'sectioned';
            $megaMenuClass .= $child->getDotsquaresMmMobHideAllcat() ? ' hide-all-category' : '';
        }
        /**
         * End Mega Menu
         */
         ...
         */
        $menuBlocks = [
            'dotsquares_mm_top_block' => false,
            'dotsquares_mm_right_block' => false,
            'dotsquares_mm_bottom_block' => false,
            'dotsquares_mm_left_block' => false,
        ];
        $submenuClass = '';
        $submenuChildClass = '';
        $blocks = [];
         ...
         */
        if ($child->getDotsquaresMmDisplayMode() != 'default') {
            $blockCode = 'dotsquares_mm_top_block';
            ...
            $blockCode = 'dotsquares_mm_left_block';
            if ($menuBlocks[$blockCode] !== false) {
                if (strpos($megaMenuClass, 'fullwidth') !== false) {
         ...
         */
        if ($child->getDotsquaresMmDisplayMode() != 'default') {
            $blockCode = 'dotsquares_mm_right_block';
            if ($menuBlocks[$blockCode] !== false) {
                if (strpos($megaMenuClass, 'fullwidth') !== false) {
            ...
            $blockCode = 'dotsquares_mm_bottom_block';
            if ($menuBlocks[$blockCode] !== false) {
                if (strpos($megaMenuClass, 'fullwidth') !== false) {
    ...
    protected function _getMenuItemClasses(\Magento\Framework\Data\Tree\Node $item)
    {
        ...
        if ($item->getLevel() == 0) {
            $displayMode = $item->getDotsquaresMmDisplayMode() ? $item->getDotsquaresMmDisplayMode() : 'sectioned';
            $classes[] = $item->getClass() . '-' . $displayMode;
            $item->getUrl() == $this->getUrl('*/*/*', ['_current' => true, '_use_rewrite' => true]) ?
                $classes[] = 'active' :
                $classes[] = '';

            if (($displayMode == 'default') && $item->getDotsquaresMmShowArrows()) {
                $classes[] = 'show-arrows';
            }
        }
        /**
    ...
    {
        if ($item->getDotsquaresMmDisplayMode() == 'boxed') {
            $columnsNumber = '1';
        } else {
            $columnsNumber = $item->getDotsquaresMmColumnsNumber() ? trim($item->getDotsquaresMmColumnsNumber()) : '4';
        }
    ...
    {
        $numbers = (float)preg_replace('/[^0-9.]*/', '', trim($item->getDotsquaresMmColumnWidth()));
        $characters = preg_replace('/[^a-zA-Z%]/', '', trim($item->getDotsquaresMmColumnWidth()));

        switch ($item->getDotsquaresMmDisplayMode()) {
            case 'fullwidth':
    ...
    protected function _getMenuBlock($blocks, $child)
    {
        ...
        foreach ($blocks as $block) {
            ...
            if ($child->getDotsquaresMmDisplayMode() == 'default' || $child->getData($block . '_type') == 'none') {
