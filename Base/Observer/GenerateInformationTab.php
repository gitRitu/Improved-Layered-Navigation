...
use Dotsquares\Base\Plugin\Backend\Model\Menu\Item;
 ...
 * @deprecated
 * @since 1.12.18
 * @see \Dotsquares\Base\Block\Adminhtml\System\Config\Information
 */
class GenerateInformationTab implements ObserverInterface
{
    ...
    {
        $html = '<div class="dotsquares-info-block">'
    ...
    {
        $src = $this->assetRepo->getUrl('Dotsquares_Base::images/amasty_logo.svg');
        if ($this->moduleInfoProvider->isOriginMarketplace()) {
            $href = Item::MAGENTO_MARKET_URL;
        } else {
            $href = 'https://www.dotsquares.com/' . $this->getSeoParams() . 'dotsquares_logo_' . $this->getModuleCode();
        }

        $html = '<a target="_blank" href="' . $href . '"><img class="dotsquares-logo" src="' . $src . '"/></a>';
        ...
        if ($content) {
            ...
            foreach ($content as $message) {
                if (isset($message['type'], $message['text'])) {
                    $html .= '<div class="dotsquares-additional-content"><span class="message ' . $message['type'] . '">'
                        . $message['text']
                        . '</span></div>';
                }
            }
    ...
    {
        $html = '<div class="dotsquares-module-version">';
    ...
    {
        $html = '<div class="dotsquares-user-guide"><span class="message success">'
            . __(
    ...
    {
        return '<div class="dsbase-buttons-container">'
            . $this->getFeatureLink()
    ...
    private function showModuleExistingConflicts()
    {
        ...
        if (count($messages)) {
            $html = '<div class="dotsquares-conflicts-title">'
                . __('Problems detected:')
                . '</div>';

            $html .= '<div class="dotsquares-disable-extensions">';
