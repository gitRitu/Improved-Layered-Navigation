...
class Builder
{
    public const BASE_MENU = 'Dotsquares_Base::menu';

    public const SETTING_ENABLE = 'dotsquares_base/menu/enable';
    ...
    private function observeMenu(Menu $menu)
    {
        ...
        foreach ($this->getInstalledModules($configItems) as $title => $installedModule) {
            ...
            if (isset($configItems[$installedModule]['id'])) {
                $dotsquaresItem = $this->generateMenuItem(
    ...
    private function cloneMenuItems($menuItems, Menu $menu)
    {
        ...
        foreach ($menuItems as $link) {
            $dotsquaresItem = $menu->get($link);
            if ($dotsquaresItem) {
                $itemData = $dotsquaresItem->toArray();
    ...
    {
        if ($this->dotsquaresItems === null) {
            $all = $this->generateDotsquaresItems($menu);
            $this->dotsquaresItems = [];
            foreach ($all as $item) {
                $name = explode('::', $item);
                $name = $name[0];
                if (!isset($this->dotsquaresItems[$name])) {
                    $this->dotsquaresItems[$name] = [];
                }
                $this->dotsquaresItems[$name][] = $item;
            }
        }

        return $this->dotsquaresItems;
    }
     ...
     * @return array
     */
    private function generateDotsquaresItems($menu)
    {
        $dotsquares = [];
        foreach ($this->getMenuIterator($menu) as $menuItem) {
            if ($this->isCollectedNode($menuItem)) {
                $dotsquares[] = $menuItem->getId();
            }
            if ($menuItem->hasChildren()) {
                foreach ($this->generateDotsquaresItems($menuItem->getChildren()) as $menuChild) {
                    $dotsquares[] = $menuChild;
                }
            }
        }
    ...
    {
        if (strpos($menuItem->getId(), 'Dotsquares') === false
            || strpos($menuItem->getId(), 'Dotsquares_Base') !== false) {
            return false;
        }
    ...
    private function getModuleTitle($name)
    {
        ...
        if ($module && isset($module['name'])) {
            ...
            $result = str_replace('Dotsquares_', '', $result);
    ...
    private function generateConfigItems()
    {
        ...
        $config = $this->findResourceChildren($configTabs, 'dotsquares');
