<?php

namespace Dotsquares\Base\Plugin\AdminNotification\Block;

use Magento\AdminNotification\Block\ToolbarEntry as NativeToolbarEntry;

/**
 * Add html attributes to DOTSQUARES notifications
 */
class ToolbarEntry
{
    public const DOTSQUARES_ATTRIBUTE = ' data-dsbase-logo="1"';

    public function afterToHtml(
        NativeToolbarEntry $subject,
        $html
    ) {
        $collection = $subject->getLatestUnreadNotifications()
            ->clear()
            ->addFieldToFilter('is_dotsquares', 1);

        foreach ($collection as $item) {
            $search = 'data-notification-id="' . $item->getId() . '"';
            if ($item->getData('image_url')) {
                $html = str_replace(
                    $search,
                    $search . ' style='
                    . '"background: url(' . $item->getData('image_url') . ') no-repeat 5px 7px; background-size: 30px;"'
                    . self::DOTSQUARES_ATTRIBUTE,
                    $html
                );
            } else {
                $html = str_replace($search, $search . self::DOTSQUARES_ATTRIBUTE, $html);
            }
        }

        return $html;
    }
}
