...
use Dotsquares\Base\Model\AdminNotification\Model\ResourceModel\Inbox\Collection\ExistsFactory;
...
class News
{
    ...
    protected function isItemValid(\SimpleXMLElement $item): bool
    {
        return $this->validateByExtension((string)$item->extension)
            && $this->validateByDotsquaresCount($item->dotsquares_module_qty)
            && $this->validateByNotInstalled((string)$item->dotsquares_module_not)
     ...
     * @return string[]
     */
    private function getInstalledDotsquaresExtensions(): array
    {
        if (!$this->dotsquaresModules) {
            $modules = $this->moduleList->getNames();
            ...
            $modules = array_filter(
                ...
                static function ($item) {
                    return strpos($item, 'Dotsquares_') !== false;
                }
            );
            $this->dotsquaresModules = $modules;
        }

        return $this->dotsquaresModules;
    }
    ...
    private function validateByExtension(string $extensions, bool $allModules = false): bool
    {
        if ($extensions) {
            ...
            if ($arrExtensions) {
                $installedModules = $allModules ? $this->getAllExtensions() : $this->getInstalledDotsquaresExtensions();
    ...
    private function validateByNotInstalled(string $extensions): bool
    {
        if ($extensions) {
            ...
            if ($arrExtensions) {
                $installedModules = $this->getInstalledDotsquaresExtensions();
     ...
     * @return bool
     */
    private function validateByDotsquaresCount($counts): bool
    {
        $result = true;
        ...
        if ($countString) {
            ...
            $dotsquaresModules = $this->getInstalledDotsquaresExtensions();
            $dependModules = $this->getDependModules($dotsquaresModules);
            $dotsquaresModules = array_diff($dotsquaresModules, $dependModules);

            $dotsquaresCount = count($dotsquaresModules);

            if ($dotsquaresCount
                && (
                    in_array($dotsquaresCount, $arrCounts, false) // non strict
                    || ($moreThan && $dotsquaresCount >= $moreThan)
                )
            ) {
                $result = true;
            }
    ...
    /**
     * @param string[] $dotsquaresModules
     *
     * @return array
     */
    private function getDependModules(array $dotsquaresModules): array
    {
        $depend = [];
        $result = [];
        $dataName = [];
        foreach ($dotsquaresModules as $module) {
            ...
            if (isset($data['require']) && is_array($data['require'])) {
                foreach ($data['require'] as $requireItem => $version) {
                    if (strpos($requireItem, 'dotsquares') !== false) {
                        $depend[] = $requireItem;
                    }
                }
            }
