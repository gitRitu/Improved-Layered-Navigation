...
namespace Dotsquares\ShopbySeo\Plugin\Framework\App\Action;

use Dotsquares\ShopbySeo\Helper\Url;
...
class Forward
{
    ...
    public function aroundDispatch(ForwardAction $subject, callable $proceed, RequestInterface $request)
    {
         ...
         */
        if ($request->getMetaData(\Dotsquares\ShopbySeo\Helper\Data::SEO_REDIRECT_FLAG) && $request->getModuleName()) {
            $request->setDispatched(true);
            return $subject->getResponse();
        } elseif ($request->getMetaData(\Dotsquares\ShopbySeo\Helper\Data::SEO_REDIRECT_MISSED_SUFFIX_FLAG)
            && in_array($request->getModuleName(), $this->suffixModules)
            && $this->urlHelper->isAddSuffixToShopby()
        ) {
            $request->setMetaData(\Dotsquares\ShopbySeo\Helper\Data::SEO_REDIRECT_FLAG, true);
