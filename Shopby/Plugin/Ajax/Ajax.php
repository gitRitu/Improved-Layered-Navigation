...
use Dotsquares\Shopby\Helper\State;
...
class Ajax
{
    public const OSN_CONFIG = 'dotsquares.xnotif.config';
    public const QUICKVIEW_CONFIG = 'dotsquares.quickview.config';
    public const SORTING_CONFIG = 'dotsquares.sorting.direction';
    public const ILN_FILTER_ANALYTICS = 'dotsquares.shopby.filter_analytics';
    ...
    protected function getAjaxResponseData($page = null)
    {
        ...
        $applyButton = $layout->getBlock('dotsquares.shopby.applybutton.sidebar');
        $tags = $this->addXTagCache($applyButton, $tags);

        $jsInit = $layout->getBlock('dotsquares.shopby.jsinit');
        $tags = $this->addXTagCache($jsInit, $tags);

        $navigationTop = null;
        if (strpos($categoryProducts, 'dotsquares-catalog-topnav') === false) {
            $navigationTop = $layout->getBlock('dsshopby.catalog.topnav');
            $tags = $this->addXTagCache($navigationTop, $tags);
        }

        $applyButtonTop = $layout->getBlock('dotsquares.shopby.applybutton.topnav');
        ...
        $responseData = [
            ...
            'h1' => $h1 ? $h1->toHtml() : '',
            'title' => $title->get(),
            'bottomCmsBlock' => $this->getBlockHtml($layout, 'dsshopby.bottom'),
        ...
        if ($layout->getBlock('category.dsshopby.ajax')) {
            $responseData['newClearUrl'] = $layout->getBlock('category.dsshopby.ajax')->getClearUrl();
        }
    ...
    /**
     * @return \Dotsquares\Shopby\Model\Layer\Cms\Manager
     */
    ...
    protected function applyEventChanges(string $html)
    {
        ...
        $this->eventManager->dispatch('dsoptimizer_process_ajax_page', ['data' => $dataObject]);
