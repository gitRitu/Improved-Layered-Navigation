...
class State extends \Magento\LayeredNavigation\Block\Navigation\State
{
    ...
    {
        return $this->getLayout()->createBlock(\Dotsquares\Shopby\Block\Navigation\State\Swatch::class)
            ->setFilter($filter)
    ...
    public function viewLabel($filter)
    {
        ...
        switch ($filterSetting->getDisplayMode()) {
            case \Dotsquares\Shopby\Model\Source\DisplayMode::MODE_IMAGES:
                $value =  $this->getSwatchHtml($filter);
                break;
            case \Dotsquares\Shopby\Model\Source\DisplayMode::MODE_TEXT_SWATCH:
                $value =  $this->getSwatchHtml($filter);
                break;
            case \Dotsquares\Shopby\Model\Source\DisplayMode::MODE_IMAGES_LABELS:
    ...
    {
        if ($filter->getFilter()->getRequestVar() == \Dotsquares\Shopby\Model\Source\DisplayMode::ATTRUBUTE_PRICE) {
    ...
    {
        $filter = $filterItem->getFilter();
        if ($filter instanceof \Dotsquares\Shopby\Model\Layer\Filter\Price && count($value) >= 2) {
            $value[0] = $value[0] ?: '';
            $value[1] = $value[1] ?: '';
    ...
    {
        if ($filter instanceof \Dotsquares\Shopby\Model\Layer\Filter\Price) {
            $value = [];
        } else {
