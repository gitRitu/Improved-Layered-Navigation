...
class DisableShopbyRoot implements DataPatchInterface
{
    ...
    {
        if ($this->moduleManager->isEnabled('Dotsquares_ShopbyRoot')) {
            $pathToModule = $this->filesystem->getDirectoryRead('app')->getAbsolutePath()
                . 'code/Dotsquares/ShopbyRoot';

            try {
                $this->moduleStatus->setIsEnabled(false, ['Dotsquares_ShopbyRoot']);
            } catch (\Exception $e) {
        ...
        $connection->delete($this->resourceConfig->getTable('setup_module'), 'module = "Dotsquares_ShopbyRoot"');
