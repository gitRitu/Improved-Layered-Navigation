...
define([
    ...
    $.widget('mage.dsShopbyFilterFromTo', $.mage.dsShopbyFilterAbstract, {
        selectors: {
            dataFromTo: '[data-dsshopby-fromto="{mode}"]',
            range: '.range'
        },
        classes: {
          range: 'range'
        ...
        _create: function () {
            ...
            this.value.on('dsshopby:sync_change', self.onSyncChange.bind(this));

            newValue = fromValue + '-' + toValue;

            this.value.trigger('dsshopby:sync_change', [[self.value.val() ? self.value.val() : newValue, true]]);
        ...
        formValidate: function () {
            ...
            self.element.find('form').mage('validation', {
                ...
                messages: {
                    'ds_shopby_filter_widget_attr_price_from': {
                        'greater-than-equals-to': message,
                        'validate-digits-range': message
                    },
                    'ds_shopby_filter_widget_attr_price_to': {
                        'greater-than-equals-to': message,
                        'validate-digits-range': message
                    }
                }
        ...
        applyFilter: function (event) {
            var valueFrom = this.processPrice(true, this.from.val()),
                ...
                linkHref = this.options.url
                    .replace('dsshopby_slider_from', fromToInterval.from.toFixed(2))
                    .replace('dsshopby_slider_to', fromToInterval.to.toFixed(2));
        ...
        onSyncChange: function (event, values) {
            ...
            for (i = 0; i < value.length; i++) {
                value[i] = this.processPrice(
                    false,
                    value[i],
                    $.mage.dsShopbyFilterAbstract.prototype.options[i === 0 ? 'deltaFrom' : 'deltaTo']
                );
            }
    ...
    return $.mage.dsShopbyFilterFromTo;
});
