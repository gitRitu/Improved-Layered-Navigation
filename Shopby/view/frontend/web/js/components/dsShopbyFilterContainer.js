/**
 *  Dotsquares Filter Container Component
 */
...
define([
    ...
    $.widget('mage.dsShopbyFilterContainer', {
        options: {
            collectFilters: 0
        },
        classes: {
        ...
        selectors: {
            ...
            filterAttrName: '[data-dsshopby-filter="attr_{name}"]',
            filterRemoveButton: '[data-container="{attr}"][data-value="{value}"] .dsshopby-remove',
            filterContainer: '[data-ds-js="shopby-container"]',
            filterActions: '.filter-actions',
            filterName: '[name="dsshopby[{name}][]"]',
            swatchOptionSelected: '.swatch-option.selected',
            filterSliderId: '[data-dsshopby-slider-id="slider"]',
            filterSliderDisplay: '[data-ds-js="slider-display"]',
            filterFromTo: '[data-dsshopby-fromto="{type}"]',
            range: '.range',
        ...
        _create: function () {
            ...
            $(links).each(function (index, value) {
                ...
                $(value).find('a').on('click', function (event) {
                    ...
                    $.mage.dsShopbyFilterAbstract.prototype.renderShowButton(event, this);
                    self.apply(filter);
                });
        ...
        apply: function (filter) {
            ...
            try {
                ...
                if ($.mage.dsShopbyAjax !== 'undefined') {
                    $.mage.dsShopbyFilterAbstract.prototype.prepareTriggerAjax(null, null, true);
        ...
        checkInForm: function (filter) {
            ...
            if (this.checkIfValueNotExist(name, value)) {
                ...
                block.append('<form class="dsshopby-saved-values" data-dsshopby-filter="attr_'
                    + name + '"><input value="' + value + '" type="hidden" name="dsshopby[' + name + '][]"></form>');
            }
        },
        ...
        checkIfValueNotExist: function (name, value) {
            ...
            filterItem.each(function (index, item) {
                if (item.multiple && $(item).hasClass('ds-select')) {
                    multiSelectOptions = [];
    ...
    return $.mage.dsShopbyFilterContainer;
});
