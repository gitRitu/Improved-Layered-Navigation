...
define([
    ...
    'mage/validation',
    'mage/translate',
    'Dotsquares_Shopby/js/jquery.ui.touch-punch.min',
    'Dotsquares_ShopbyBase/js/chosen/chosen.jquery',
    'dsShopbyFiltersSync'
], function ($) {
    'use strict';

    $.widget('mage.dsShopbyFilterAbstract', {
        filters: {},
        options: {
            isAjax: 0,
            collectFilters: 0,
        ...
        selectors: {
            fromToWidget: '[data-Dotsquares-js="fromto-widget"]',
            filterForm: 'form[data-dsshopby-filter]',
            filterFormAttr: 'form[data-dsshopby-filter={attr}]',
            filterItems: '[class*="Dotsquares-filter-items"]',
            sidebar: '.sidebar',
            topNav: '.catalog-topnav',
            filterOptionsContent: '.filter-options-content',
            filterOptionsItem: '.filter-options-item',
            ajaxOptionsSelectors: 'body.page-with-filter, body.catalogsearch-result-index, body.cms-index-index',
            filterRequestVar: '[data-dsshopby-filter-request-var="cat"]',
            dFrom: 'df=',
            dTo: 'dt='
        },
        classes: {
            active: '-active',
            disabled: '-disabled',
            savedFilterValues: 'dsshopby-saved-values',
            selected: 'dsshopby-link-selected'
        },
        ...
        getFilter: function () {
            return {
                'code': this.element.attr('dsshopby-filter-code'),
                'value': this.element.attr('dsshopby-filter-value')
            };
        },
        ...
        apply: function (link, clearFilter) {
            ...
            try {
                if ($.mage.dsShopbyAjax) {
                    $.mage.dsShopbyAjax.prototype.response = null;
                }

                this.options.isAjax = $.mage.dsShopbyAjax !== 'undefined';
        ...
        prepareTriggerAjax: function (element, clearUrl, clearFilter, isSorting) {
            var self = this,
                widgetInstance = $.mage.dsShopbyFilterAbstract.prototype,
            ...
            forms.each(function (index, item) {
                ...
                startPos = className.indexOf('ds-filter-items');
            ...
            $(element).trigger('dsshopby:submit_filters', {
                data: data,
        ...
        normalizeData: function (data, isSorting, clearFilter) {
            var self = this,
                normalizedData = [],
                ajaxOptions = $(this.selectors.ajaxOptionsSelectors).dsShopbyAjax('option'),
                clearUrl;
                ...
                if (item && item.value.trim() !== '' && item.value !== '-1') {
                    var isNormalizeItem = _.find(normalizedData, function (normalizeItem) {
                        return normalizeItem.name === item.name && normalizeItem.value === item.value
                            || item.name === 'dsshopby[price][]' && normalizeItem.name === item.name;
                    });
        ...
        renderShowButton: function (event, element) {
            if ($.mage.dsShopbyApplyFilters) {
                $.mage.dsShopbyApplyFilters.prototype.renderShowButton(event, element);
            }
        },
        ...
        getDeltaParams: function (from, valueFrom, to, valueTo, isUrl) {
            ...
            $.mage.dsShopbyFilterAbstract.prototype.options.deltaFrom = deltaFrom;
            $.mage.dsShopbyFilterAbstract.prototype.options.deltaTo = deltaTo;
            ...
            if (isUrl) {
                ...
                return [
                    {name: 'dsshopby[df][]', value: deltaFrom},
                    {name: 'dsshopby[dt][]', value: deltaTo}
                ]
            }
        },
        ...
        setDeltaParams: function (delta) {
            $.mage.dsShopbyFilterAbstract.prototype.options.delta = delta instanceof Array ? delta : [];
        },
    ...
    return $.mage.dsShopbyFilterAbstract;
});
