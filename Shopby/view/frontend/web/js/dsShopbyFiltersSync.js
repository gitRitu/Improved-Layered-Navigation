define([
    ...
    return function (options) {
        var beforeValue = [];
        $(document).on("sync", "[data-dsshopby-filter]", function (event, clearFilter) {
            syncFilters(this, clearFilter);
        });

        function syncFilters(element, clearFilter) {
            var currentForm = $(element);
            var attributeCode = currentForm.attr('data-dsshopby-filter');

            $('[data-dsshopby-filter="' + attributeCode + '"]').each(function () {
                if (this !== currentForm.get(0) || clearFilter) {
                    beforeValue = $(this).serializeArray();

                    var data = normalizeData(currentForm.serializeArray());
                    delete data['dsshopby[attr_price_from][]'];
                    delete data['dsshopby[attr_price_to][]'];
                    ...
                    _(data).each(function (values, name) {
                        ...
                        if (values[0] || clearFilter) {
                            element.val(values);
                            element.trigger("dsshopby:sync_change", [values]);
