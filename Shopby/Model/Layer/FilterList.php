...
use Dotsquares\Base\Model\MagentoVersion;
use Dotsquares\Shopby\Helper\Config;
use Dotsquares\Shopby\Helper\FilterSetting;
use Dotsquares\Shopby\Model\Layer\Filter\Category;
...
class FilterList extends Layer\FilterList
{
    ...
    public const ALL_FILTERS_KEY  = 'dotsquares_shopby_all_filters';
    ...
    protected function getAdditionalFilters(Layer $layer)
    {
        ...
        if ($this->isCustomFilterEnabled('stock') && $this->config->isEnabledShowOutOfStock()) {
            $additionalFilters[] = $this->objectManager->create(
                \Dotsquares\Shopby\Model\Layer\Filter\Stock::class,
                ['layer' => $layer]
            );
        }
        ...
        if ($this->isCustomFilterEnabled('rating')) {
            $additionalFilters[] = $this->objectManager->create(
                \Dotsquares\Shopby\Model\Layer\Filter\Rating::class,
                ['layer' => $layer]
            );
        }
        ...
        if ($this->isCustomFilterEnabled('am_is_new')) {
            $additionalFilters[] = $this->objectManager->create(
                \Dotsquares\Shopby\Model\Layer\Filter\IsNew::class,
                ['layer' => $layer]
            );
        }
        ...
        if ($this->isCustomFilterEnabled('am_on_sale')) {
            $additionalFilters[] = $this->objectManager->create(
                \Dotsquares\Shopby\Model\Layer\Filter\OnSale::class,
                ['layer' => $layer]
            );
        }
