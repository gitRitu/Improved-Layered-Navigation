...
use Dotsquares\Shopby\Model\Layer\Filter\OnSale\Helper as OnSaleHelper;
...
class OnSale implements OperationInterface
{
    ...
    public function getAggregation(Table $entityIdsTable, array $dimensions = []): Select
    {
        ...
        $derivedTable->joinLeft(
            ['ds_on_sale' => $collection->getSelect()],
            'ds_on_sale.entity_id  = entities.entity_id',
            [
                'value' => new \Zend_Db_Expr("if(ds_on_sale.entity_id is null, 0, 1)")
            ]
        );
