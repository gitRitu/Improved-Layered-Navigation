...
use Dotsquares\ShopbyBase\Helper\FilterSetting as FilterSettingHelper;
use Dotsquares\ShopbyBase\Helper\FilterSetting;
use Dotsquares\ShopbyBase\Model\ResourceModel\OptionSetting\CollectionFactory as OptionCollectionFactory;
...
class Data extends AbstractHelper
{
    ...
    public const PATH_BRAND_URL_KEY = 'dsshopby_brand/general/url_key';
    ...
    /**
     * @var \Dotsquares\ShopbyBrand\Model\Attribute
     */
    ...
    public function __construct(
        ...
        \Dotsquares\ShopbyBrand\Model\Attribute $brandAttribute
    ) {
    ...
    private function getIdentifier($scopeCode)
    {
        $pageIdentifier = $this->scopeConfig->getValue(
            'dsshopby_brand/general/brands_page',
    ...
    public function getSuffix()
    {
        ...
        if ($this->scopeConfig->isSetFlag('dotsquares_shopby_seo/url/add_suffix_shopby')) {
    ...
    {
        return $this->scopeConfig->getValue('dsshopby_brand/' . $path, ScopeInterface::SCOPE_STORE) ?: '';
    }
    ...
    /**
     * @param array|\Dotsquares\ShopbyBrand\Model\Brand\BrandDataInterface $item
     * @return string
     */
    public function generateToolTipContent($item)
    {
        ...
        if ($template) {
            ...
            $content = 'data-dsshopby-js="brand-tooltip" data-tooltip-content="' . $template . '"';
        }
            ...
            foreach ($matches[1] as $match) {
                ...
                switch ($match) {
                    ...
                    case 'image':
                        ...
                        if (isset($imgUrl)) {
                            $value = sprintf('<img class=\'ds-brand-%s\' src=\'%s\'>', $match, $imgUrl);
                        }
                        break;
                }
    ...
    {
        return $this->_moduleManager->isEnabled('Dotsquares_ShopbySeo')
            ? $this->scopeConfig->getValue('dotsquares_shopby_seo/url/special_char', ScopeInterface::SCOPE_STORE)
            : '-';
    }
    ...
    public function getBrandLabel()
    {
        return (string) $this->scopeConfig->getValue(
            'dsshopby_brand/general/menu_item_label',
            ScopeInterface::SCOPE_STORE
        );
    }
