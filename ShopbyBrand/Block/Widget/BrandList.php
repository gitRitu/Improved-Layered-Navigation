...
class BrandList extends BrandListAbstract implements BlockInterface
{
     ...
     */
    public const CONFIG_VALUES_PATH = 'dsshopby_brand/brands_landing';
    ...
    {
        $block = $this->getLayout()->getBlock('dsbrands.search');
        if (!$block) {
            $block = $this->getLayout()->createBlock(Template::class, 'dsbrands.search')
                ->setTemplate('Dotsquares_ShopbyBrand::brand_search.phtml');
        }
