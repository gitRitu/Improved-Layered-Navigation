...
class CreateAllBrandsCmsPage implements DataPatchInterface
{
    ...
    {
        if (!$this->config->getValue('dsshopby_brand/general/brands_page')) {
...
<p>{{widget type="Dotsquares\ShopbyBrand\Block\Widget\BrandSlider" template="widget/brand_list/slider.phtml"}}</p>
<p>{{widget type="Dotsquares\ShopbyBrand\Block\Widget\BrandList" columns="3"
template="widget/brand_list/index.phtml"}}</p>';
            ...
            $page->setTitle('All Brands Page')
                ...
                ->setContent($content)
                ->save();
            $this->configWriter->save('dsshopby_brand/general/brands_page', $identifier);
        }
